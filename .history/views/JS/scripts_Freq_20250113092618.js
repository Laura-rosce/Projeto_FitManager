...
async function entrarFrequencia() {
  ...
  try {
      const response = await fetch(`${url}/frequencia/entrar`, {  
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ usuarioAluno: usuarioInput, senhaAluno: senhaInput })  
      });

      if (response.ok) {
          const data = await response.json();  

          alert(`Frequência registrada para o aluno: ${data.nomeAluno}`);
          
          // Adiciona o botão de usuário na barra lateral
          const barraLateral = document.querySelector(".sidebar");

          // Cria o botão do usuário com nome e status
          const botaoUsuario = document.createElement("button");
          botaoUsuario.className = "btnFre mb-1 w-85";
          botaoUsuario.innerHTML = `${data.nomeAluno}<br>Ativo`;
          botaoUsuario.id = `botao-${usuarioInput}`;
          
          barraLateral.appendChild(botaoUsuario);

          // Configura o estilo da barra lateral para permitir overflow
          barraLateral.style.overflowY = "auto";
          barraLateral.style.maxHeight = "100vh"; // Define altura máxima para a barra lateral
          barraLateral.style.overflowX = "hidden"; // Impede rolagem horizontal
