...
async function entrarFrequencia() {
    const usuarioInput = document.getElementById("user").value.trim(); //perguntar ao chat
    ...
    try {
      const response = await fetch(`${url}/frequencia/entrar`, {  // Chama a API para registrar a frequência
        method: 'POST',
      ...
      if (response.ok) {
        const data = await response.json();  // Analisa o JSON diretamente
        ...
        botaoUsuario.className = "btnFre mb-1 w-80";
        ...
        const errorData = await response.json();  // Analisa o JSON do erro
